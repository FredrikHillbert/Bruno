...
export function ApiKeyModal({ isOpen, onClose, onSave, currentKeys }: ApiKeyModalProps) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Add your API keys to access different AI providers. Keys are stored locally and never sent to our servers.
          </DialogDescription>
        </DialogHeader>

        <Alert>
          <AlertDescription>
            <strong>Bring Your Own Key (BYOK)</strong> - Use your own API keys for unlimited access. You only pay for
            what you use directly to the AI providers.
          </AlertDescription>
        </Alert>
            ...
            <TabsContent key={provider.id} value={provider.id} className="space-y-4">
                ...
                <p>
                  ...
                  <Button variant="link" className="p-0 h-auto ml-1" asChild>
                    <a href={provider.url} target="_blank" rel="noopener noreferrer">
                      Get one from {provider.name} <ExternalLink className="h-3 w-3 ml-1" />
                    </a>
                  </Button>
                </p>
              </div>
              ...
              <div className="flex gap-2">
                ...
                {currentKeys[provider.id] && (
                  <Button variant="outline" onClick={() => handleRemove(provider.id)}>
                    Remove
                  </Button>
                )}
              </div>
